...
test("golden path (sandbox)", { skip: !amen && "AMN_API_KEY not set", timeout: 120_000 }, async () => {
  ...
  const deal = await amen.deals.create({ offer_type: "product", offer_category: (await amen.lookups.categories())[0].id, offer_title: "Starter Kit golden path", offer_description: "Reference deal created by the Amen integration starter kit", offer_price: "100.00", offer_delivery_fee: "0.00" });
