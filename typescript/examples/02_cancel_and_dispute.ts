/** npx tsx examples/02_cancel_and_dispute.ts cancel | dispute DL-000123 (deal must be 'completed') */
import { AmenClient } from "../src/amen/index.js";
import { step, uniquePhone } from "./_common.js";
const amen = new AmenClient();
const [mode = "cancel", dealNo] = process.argv.slice(2);
if (mode === "cancel") {
  ...
  const deal = await amen.deals.create({ offer_type: "service", offer_title: "Cancel scenario", offer_price: "50.00",
    offer_category: (await amen.lookups.categories())[0].id, deal_subject_details: "Kit test" });
